...
export function TicketsPage() {
  ...
  for (let i = 1; i <= 40; i++) {
    ...
    const mod = i % 7
    if (i <= 12 && mod !== 3) { bg = '#0B0B0B'; fg = '#B4F000' }
    else if (mod === 3) { bg = '#FF3A2D'; fg = '#fff' }
    else { border = '1.4px solid rgba(11,11,11,.18)' }
    tickets.push({ n: i, bg, fg, border })
  }

  const filtered = search.trim()
    ? tickets.filter(t => String(t.n).includes(search.trim()))
    : tickets
    ...
    <div className="noscroll" style={{ position: 'absolute', inset: 0, background: '#B4F000', color: '#0B0B0B', overflowY: 'auto', padding: '62px 24px 40px' }}>
      ...
      <div style={{ height: 52, borderRadius: 999, background: 'rgba(11,11,11,.08)', display: 'flex', alignItems: 'center', gap: 10, padding: '0 20px', marginBottom: 22 }}>
        ...
        <input
          ...
          placeholder="Поиск билета"
          style={{ background: 'none', border: 'none', outline: 'none', fontSize: 15, color: '#0B0B0B', width: '100%', fontFamily: "'Geist',sans-serif" }}
        />
        {search && <button onClick={() => setSearch('')} style={{ background: 'none', border: 'none', color: 'rgba(11,11,11,.4)', fontSize: 18, cursor: 'pointer', padding: 0, lineHeight: 1 }}>×</button>}
      </div>
      {!search && (
      ...
      <div style={{ fontFamily: "'Geist Mono',monospace", fontSize: 11, letterSpacing: 1.5, textTransform: 'uppercase', color: 'rgba(11,11,11,.55)', marginBottom: 14 }}>
        {search ? `Результаты: ${filtered.length}` : 'Все 40 билетов'}
      </div>
